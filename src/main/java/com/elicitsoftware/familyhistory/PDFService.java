...
public class PDFService {
    ...
    public byte[] generatePDF(long respondentId) {
        try {
            ...
            for (ReportDefinition rpt : respondent.survey.reports) {
                reportResponse = callReport(rpt, respondentId);
    ...
    private void generateContentStream(Table table) throws IOException {
        ...
        contentStream.close();
        contentStream = new PDPageContentStream(document, page);
        // No current report produces a landscape table (createContent always builds
        // portrait tables, and the only landscape=true report never has a Content.table),
        // so this new-page geometry is unverified for landscape - revisit if that changes.
     ...
     * @param currentPageRowHeights each row's actual height, matching {@code currentPageContent}
     * @throws IOException if an error occurs while drawing
     */
    ...
    /**
     * Writes a single line of table content, wrapping any cell whose text is wider than its
     ...
     *
     * @param table the table being drawn
     * @param currentPageRowHeights each body row's actual height for this page
     ...
     *
     * @param table the table being drawn
     * @param y the Y coordinate to draw the line at
     * @throws IOException if an error occurs while drawing
     */
